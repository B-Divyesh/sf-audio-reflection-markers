...
Open `/?demo=1`, `/demo`, or `/demo/` to load two sample markers from MIT OpenCourseWare’s public **Algorithms and Computation** lecture. The `demo-sample-content` claim test checks the count and recall cue from a fresh `/demo` context. The saved reference is `https://www.youtube.com/watch?v=ZA-tUyM_y7s`; rendered timestamp links add YouTube’s `t=` parameter and are checked for a successful response. On a phone, the first complete marker and cue appear above the source form. The persistent banner identifies sample mode and offers **Reset demo** and **Start for real**.
